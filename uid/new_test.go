...
func TestNanoWithPrefix(t *testing.T) {
	...
	id := New(RequestPrefix)

	if !strings.HasPrefix(id, string(RequestPrefix)) {
		t.Errorf("Expected ID to start with prefix %v, got %v", RequestPrefix, id)
	}

	// Total length should be prefix + 8 default chars
	expectedLen := len(RequestPrefix) + 1 + 8
