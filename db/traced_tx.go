...
func WrapTxWithContext(tx *sql.Tx, mode string, ctx context.Context) DBTx {
	return wrapTxWithMetrics(tx, mode, ctx, metrics.New(""))
}

func wrapTxWithMetrics(tx *sql.Tx, mode string, ctx context.Context, metricSet *metrics.Metrics) DBTx {
	return &TracedTx{
		tx:      tx,
		mode:    mode,
		ctx:     ctx,
...
func (t *TracedTx) ExecContext(ctx context.Context, query string, args ...any) (sql.Result, error) {
	...
	t.recordMetric("exec", status, duration)
...
func (t *TracedTx) PrepareContext(ctx context.Context, query string) (*sql.Stmt, error) {
	...
	t.recordMetric("prepare", status, duration)
...
func (t *TracedTx) QueryContext(ctx context.Context, query string, args ...any) (*sql.Rows, error) {
	...
	t.recordMetric("query", status, duration)
...
func (t *TracedTx) QueryRowContext(ctx context.Context, query string, args ...any) *sql.Row {
	...
	t.recordMetric("query_row", status, duration)
...
func (t *TracedTx) Commit() error {
	...
	t.recordMetric("commit", status, duration)
...
func (t *TracedTx) Rollback() error {
	...
	t.recordMetric("rollback", status, duration)

	return err
}

func (t *TracedTx) recordMetric(operation, status string, duration float64) {
	t.metrics.Observe(t.mode, operation, status, duration)
}
